...
void buildTree(int a[][4], int s, int e, int tree[][4], int index, int r)
{
    ...
    tree[index][0] =
        ((tree[2 * index][0] * tree[2 * index + 1][0]) + (tree[2 * index][1] * tree[2 * index + 1][2])) % r;
    tree[index][1] =
        ((tree[2 * index][0] * tree[2 * index + 1][1]) + (tree[2 * index][1] * tree[2 * index + 1][3])) % r;
    tree[index][2] =
        ((tree[2 * index][2] * tree[2 * index + 1][0]) + (tree[2 * index][3] * tree[2 * index + 1][2])) % r;
    tree[index][3] =
        ((tree[2 * index][2] * tree[2 * index + 1][1]) + (tree[2 * index][3] * tree[2 * index + 1][3])) % r;
    // cout << endl << index << "---->" << tree[index][0] << tree[index][1] <<
