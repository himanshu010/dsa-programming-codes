...
// void updateRange(int *tree, int ss, int se, int l, int r, int  increment, int
// index ) {
//     if (l > se || r < ss) {
//         return;
//     }
...
void buildTree(int a[][4], int s, int e, int tree[][4], int index, int r) {
  ...
  if (s == e) {
    ...
    tree[index][2] = a[s][2];
    tree[index][3] = a[s][3];
    // cout << endl << index << "---->" << tree[index][0] << tree[index][1] <<
    // tree[index][2] << tree[index][3] << endl;
    return;
  }
  // Rec case
  int mid = (s + e) / 2;
  ...
  tree[index][0] = ((tree[2 * index][0] * tree[2 * index + 1][0]) +
                    (tree[2 * index][1] * tree[2 * index + 1][2])) %
                   r;
  tree[index][1] = ((tree[2 * index][0] * tree[2 * index + 1][1]) +
                    (tree[2 * index][1] * tree[2 * index + 1][3])) %
                   r;
  tree[index][2] = ((tree[2 * index][2] * tree[2 * index + 1][0]) +
                    (tree[2 * index][3] * tree[2 * index + 1][2])) %
                   r;
  tree[index][3] = ((tree[2 * index][2] * tree[2 * index + 1][1]) +
                    (tree[2 * index][3] * tree[2 * index + 1][3])) %
                   r;
  // cout << endl << index << "---->" << tree[index][0] << tree[index][1] <<
  // tree[index][2] << tree[index][3] << endl; cout << tree[2 * index][0] << "*"
  // << tree[2 * index + 1][0] << "  +  " << tree[2 * index][1] << "*" << tree[2
  // * index + 1][2];
  return;
}
vector<int> leftAns, rightAns;
vector<int> queryAns = {1, 0, 0, 1};
vector<int> query(int tree[][4], int ss, int se, int qs, int qe, int index,
                  int r) {
  if (ss >= qs && se <= qe) {
    vector<int> v;
    v.pb(tree[index][0]);
    ...
    v.pb(tree[index][2]);
    v.pb(tree[index][3]);
    // cout << endl << index << "---->" << queryAns[0] << queryAns[1] <<
    // queryAns[2] << queryAns[3] << endl; cout << "=====" << queryAns[2] <<
    // tree[index][1] << ((queryAns[2] * tree[index][1])) << (queryAns[3] *
    // tree[index][3]) << ((queryAns[2] * tree[index][1]) + (queryAns[3] *
    // tree[index][3]));
    ...
    // cout << endl << index << "---->" << tree[index][0] << tree[index][1] <<
    // tree[index][2] << tree[index][3] << endl; cout << endl << index <<
    // "---->" << queryAns[0] << queryAns[1] << queryAns[2] << queryAns[3] <<
    // endl; cout << "=====" << queryAns[2] << tree[index][1] << ((queryAns[2] *
    // tree[index][1])) << (queryAns[3] * tree[index][3]) << ((queryAns[2] *
    // tree[index][1]) + (queryAns[3] * tree[index][3])); cout << endl << endl;
    return v;
  }
  ...
  // cout << index << "llll---->" << leftAns[0] << leftAns[1] << leftAns[2] <<
  // leftAns[3] << endl; cout << index << "rrrr---->" << rightAns[0] <<
  // tree[index][1] << tree[index][2] << tree[index][3] << endl;
  vector<int> v(4);
  // v.pb(1);
...
int32_t main() {
  ...
  // int t;cin>>t;while(t--)
  {
    ...
    //     cout << one << ")." << endl;
    //     one++;
    //     cout << tree[i][0] << tree[i][1] << endl << tree[i][2] << tree[i][3]
    //     << endl << endl;
    // }
    ...
    for (int i = 0; i < q; ++i) {
      ...
      // cout << "ANSWER";
      cout << queryAns[0] << " " << queryAns[1] << endl
           << queryAns[2] << " " << queryAns[3] << endl;
      queryAns = {1, 0, 0, 1};
      cout << endl;
    }
